...
pub struct Request {
    ...
    /// mode. Development and production modes impact the execution of Substreams, important
    /// aspects of execution include:
    ...
    /// * The time required to reach the first byte in development mode is faster than in
    ///   production mode.
    ///
    /// Specific attributes of development mode include:
    /// * The client will receive all of the executed module's logs.
    /// * It's possible to request specific store snapshots in the execution tree (via
...
pub struct ModuleStats {
    ...
    /// total_store_operation_time_ms is the sum of all time spent running that module code waiting
    /// for a store operation (ex: read, write, delete...)
    #[prost(uint64, tag = "5")]
    pub total_store_operation_time_ms: u64,
    /// total_store_read_count is the sum of all the store Read operations called from that module
    /// code
    #[prost(uint64, tag = "6")]
    pub total_store_read_count: u64,
    /// total_store_write_count is the sum of all store Write operations called from that module
    /// code (store-only)
    #[prost(uint64, tag = "10")]
    pub total_store_write_count: u64,
    /// total_store_deleteprefix_count is the sum of all store DeletePrefix operations called from
    /// that module code (store-only) note that DeletePrefix can be a costly operation on large
    /// stores
    #[prost(uint64, tag = "11")]
    pub total_store_deleteprefix_count: u64,
    /// store_size_bytes is the uncompressed size of the full KV store for that module, from the
    /// last 'merge' operation (store-only)
    #[prost(uint64, tag = "12")]
    pub store_size_bytes: u64,
    /// total_store_merging_time_ms is the time spent merging partial stores into a full KV store
    /// for that module (store-only)
    #[prost(uint64, tag = "13")]
    pub total_store_merging_time_ms: u64,
    /// store_currently_merging is true if there is a merging operation (partial store to full KV
    /// store) on the way.
    #[prost(bool, tag = "14")]
    pub store_currently_merging: bool,
    /// highest_contiguous_block is the highest block in the highest merged full KV store of that
    /// module (store-only)
    #[prost(uint64, tag = "15")]
